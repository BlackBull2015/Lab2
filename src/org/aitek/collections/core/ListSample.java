...
public class ListSample extends CollectionSample implements PropertyChangeListener {
	...
	public ListSample(StatsPanel statsPanel, Main main) {
		...
		Rolelis = new RoleList(Constants.COLLECTION_MAX_SIZE * 1000);
	...
	private class Task extends SwingWorker<Void, Void> {
		...
		private long[] removeFromLists() {
			...
			for (int z = 0; z <= iterations; z++) {
				...
				for (int j = 0; j < 10; j++)
					Rolelis.remove(0);
				for (int j = 0; j < 10; j++)
					Rolelis.remove(Rolelis.size() / 2);
				for (int j = 0; j < 10; j++)
					Rolelis.remove(Rolelis.size()-1);
				times[1] += System.nanoTime() - startingTime;
		...
		private long[] searchLists() {
			...
			for (int z = 0; z <= iterations; z++) {
				...
				for (int j = 0; j < 10; j++) {
					Rolelis.get(0);
					Rolelis.get(Rolelis.size() / 2);
					Rolelis.get(Rolelis.size() - 1);
				}
				times[1] += System.nanoTime() - startingTime;
